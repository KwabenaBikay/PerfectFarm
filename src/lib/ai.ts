...
export async function analyzeCropImage(base64Data: string, mimeType: string, userProfile?: UserProfile | null): Promise<CropAnalysisResult> {
  let profileContext = "";
...
If the image is too blurry, or does not contain a plant, set 'possible_disease' to 'Invalid Image', 'crop_detected' to 'Unknown', and provide appropriate advice in the explanation.`;
  ...
  return JSON.parse(response.text) as CropAnalysisResult;
}
